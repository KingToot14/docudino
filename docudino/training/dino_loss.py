...
class DINOLoss(nn.Module):
    def __init__(self, d_model: int, n_crops: int, t_temp: float,
                 warmup_t_temp: float, warmup_t_epochs: int, s_temp: float,
                 epochs: int, center_momentum: float):
        """
        Args:
            d_model (int): The embedding size of the model
        ...
        self.register_buffer("center", torch.ones(1, d_model))
    
    def forward(self, s_out: torch.Tensor, t_out: torch.Tensor, epoch: int) -> float:
        # split model output
        s_out = s_out / self.student_temp
        s_out = s_out.chunk(self.n_crops)
        
        temp = self.teacher_temp[epoch]
        t_out = F.softmax((t_out - self.center) / temp, dim=-1)
        t_out = t_out.detach().chunk(2) # detach to prevent updating gradients
        
        # calculate loss
        total_loss = 0
        n_loss_terms = 0
        
        for t in range(len(t_out)):
            for s in range(len(s_out)):
                # skip if views are the same
                if t == s:
                    continue
                
                # calculate loss
                loss = torch.sum(-t_out[t] * F.log_softmax(s_out[s], dim=-1), dim=-1)
